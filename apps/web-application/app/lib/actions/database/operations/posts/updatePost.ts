...
// Assumption: fileSchema has been defined in '@repo/db' and includes fields:
// usageType: String
// filePath: String (required)
// mimeType: String (for storing the MIME type of the file)
// user: mongoose.Schema.Types.ObjectId (reference to the user who uploaded the file)
// Example structure for fileSchema in '@repo/db':
/*
...
const updatePost = async (formData: FormData): Promise<ServerActionResponse<{ postId: string } | null>> => {
  try {
    ...
    if (thumbnailFile) {
      if (currentPostId) {
        const existingPost = await postSchema.findById(currentPostId).select('thumbnail').populate('thumbnail').lean();
        if (existingPost?.thumbnail && typeof existingPost.thumbnail === 'object' && (existingPost.thumbnail as any)._id) {
          ...
          if (oldThumb.filePath) {
            try {
              await unlink(path.join(process.cwd(), oldThumb.filePath));
            } catch (e: any) {
    ...
    if (!currentPostId) {
      ...
      const postToEdit = await postSchema.findById(currentPostId).select('author').lean();
      if (!postToEdit) {
        return errorResponse({ message: "Post not found." });
      }
      if (postToEdit.author.toString() !== userId.toString()) {
        // Add role-based check here if admins/moderators can edit other users' posts
        return errorResponse({ message: "You are not authorized to edit this post." });
      }
      if (dataToSave.author && dataToSave.author.toString() !== postToEdit.author.toString()) {
...
// import { IPost } from '@repo/typescript-types/src/Post';
//
// const updatePost = async ({ data }: IUpdatePost) => {
//   try {
//     const { isAuth,userId } = await verifySession();
//
//     if (!isAuth) {
