...
const removeThePost = async (_id: string) => {
  try {
    const removingPost = await postSchema
      .findByIdAndUpdate(_id, { $set: { status: 'trash' } }, { new: true })
      .lean()
      .exec();
    const metas = [...(removingPost?.tags || []), ...(removingPost?.categories || []), ...(removingPost?.categories || [])];
    for await (const meta of metas) {
      await metaSchema.findByIdAndUpdate(meta, { $inc: { count: -1 } }).exec();
    }
  } catch (error) {}
};
