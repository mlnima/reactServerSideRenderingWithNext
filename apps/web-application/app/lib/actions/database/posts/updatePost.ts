...
const updatePost = async (formData: FormData): Promise<ServerActionResponse<{ postId: string } | null>> => {
  ...
  try {
    ...
    connection = await connectToDatabase('updatePost');
    ...
    try {
      ...
      if (thumbnailFile) {
        if (currentPostId) {
          const existingPost = await postSchema.findById(currentPostId).select('thumbnail').populate('thumbnail').session(session).lean<IPost>();
          if (existingPost?.thumbnail && typeof existingPost.thumbnail === 'object' && (existingPost.thumbnail as any)._id) {
            ...
            await fileSchema.findByIdAndDelete(oldThumb._id).session(session);
          }
        }
        ...
        const savedFileDoc = await newFileDoc.save({ session });
      ...
      if (!currentPostId) {
        ...
        const postToEdit = await postSchema.findById(currentPostId).select('author').session(session).lean<IPost>();
        if (!postToEdit) {
      ...
      if (currentPostId) {
        ...
        const updatedPost = await postSchema.findByIdAndUpdate(currentPostId, { $set: dataToSave }, { new: true, runValidators: true, session });
        if (!updatedPost) {
        ...
        const savedPost = await newPostInstance.save({ session });
