...
const updatePost = async (formData: FormData): Promise<ServerActionResponse<{ postId: string } | null>> => {
  try {
    ...
    if (thumbnailFile) {
      if (currentPostId) {
        const existingPost = await postSchema
        ...
        if (existingPost?.thumbnail && typeof existingPost.thumbnail === 'object' && (existingPost.thumbnail as any)._id) {
          ...
          await fileSchema.findByIdAndDelete(oldThumb._id).exec();
        }
      }
    ...
    if (!currentPostId) {
      ...
      const countUserPendingPosts = await postSchema
        .countDocuments({
      ...
      const postToEdit = await postSchema.findById(currentPostId).select('author').lean<IPost>().exec();
      if (!postToEdit) {
