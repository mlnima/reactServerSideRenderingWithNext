...
const MessengerConversationPreview: FC<IProps> = ({ conversationData }) => {
    ...
    useEffect(() => {
        if (conversationData?.users?.length > 0) {
            setPartnerData(conversationData?.users?.filter(user => user._id !== userData?._id)[0]);
        }
    }, [conversationData]);
    ...
    return (
        <div className={'messengerConversationPreview'}>
            <div
                className={'messenger-conversation-preview'}
                    ...
                    <div className={'seen-content'}>
                        <span>
                            <FontAwesomeIcon
                                ...
                                color={
                                    conversationData?.messages?.[0]?.isRead
                                        ? '#3376d0'
                                        : 'var(--secondary-text-color,#b3b3b3)'
                                }
                            />
                        </span>
                        <p className={'content'}>{conversationData?.messages?.[0]?.content || ''}</p>
                    </div>
                </div>
