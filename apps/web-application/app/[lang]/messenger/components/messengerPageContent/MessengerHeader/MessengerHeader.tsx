...
const MessengerHeader: FC<IProps> = ({ activeConversation, autoScroll, setAutoScroll }) => {
    ...
    useEffect(() => {
        if (activeConversation?.users?.length > 0) {
            setPartnerData(activeConversation?.users?.filter((user: User) => user._id !== userData?._id)[0]);
        }
    }, [activeConversation]);
            ...
            {!!activeConversation?._id && (
                <div className="messengerHeaderContent">
                    <div className="messengerHeaderLeftContent">
                        <button onClick={() => push('/messenger')} className={'btn btn-transparent mobile-only'}>
                        ...
                        <p
                            onClick={() => router.push(`/user/${partnerData?.username || ''}`)}
                            className="messenger-conversation-header-username"
                        >
