...
const ChatroomTopbar: FC<IProp> = ({ chatrooms, chatroomId, autoScroll, setAutoScroll, preference, updatePreference, locale }) => {
  ...
  return (
    <div className={'chatroomTopbar'}>
      ...
      <select className={'primarySelect chatroom-selector'} onChange={(e) => onChatroomChangeHandler(e)} value={chatroomId}>
        {(chatrooms || []).map((chatroom) => {
          return (
            <option value={chatroom._id} key={chatroom._id}>
      ...
      <div className={'chatroomActionsButtons'}>
        <button onClick={() => setAutoScroll(!autoScroll)} className={`topBarActionButton ${autoScroll && 'topBarActionButtonActive'}`}>
        ...
        {/*</button>*/}
        <button
          className={`topBarActionButton ${preference.onlineUserListVisibility && 'topBarActionButtonActive'}`}
