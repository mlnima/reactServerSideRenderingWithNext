...
import { convertMetasTypeToSingular, capitalizeFirstLetters } from '@repo/utils/dist/src';
...
const PostActor: FC<ComponentPropTypes> = ({ item }) => {
  return (
    <Link href={`/${convertMetasTypeToSingular(item.type)}/${item._id}`} className={`post-meta-item ${item.type}`} title={item.name}>
