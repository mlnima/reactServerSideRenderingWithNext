  ...
  if (metas?.length) {
    return (
      <div className={`post-meta`}>
        {type !== 'all' && !!type && (
          <h2 className={'title'}>{dictionary?.[capitalizeFirstLetters(type)] || capitalizeFirstLetters(type)}:</h2>
        )}
        <div className="content">
          {metas.map((item) =>
            item.type === 'actors' ? <PostActor key={item._id} item={item} /> : <PostMeta key={item._id} item={item} />,
          )}
        </div>
      </div>
    );
