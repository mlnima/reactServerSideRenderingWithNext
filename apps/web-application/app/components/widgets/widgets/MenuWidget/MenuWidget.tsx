...
const MenuWidget: FC<MenuWidgetPropTypes> = ({ menuItems, dictionary, locale, uniqueData }) => {
    const [open, setOpen] = useState(false);
    const dispatch = useAppDispatch();
    const MenuWidgetRef = useRef(null);
    const { backgroundFilter } = useAppSelector(({ globalState }) => globalState);

    const { logoUrl, logoWidth, logoHeight } = useAppSelector(
        ({ settings }) => settings?.initialSettings?.layoutSettings,
    );
    ...
    const renderMenuItems = [
        ...(menuItems || [])
            .filter((menuItem: MenuItem) => !menuItem.parent)
            .sort((a, b) => (a.itemIndex > b.itemIndex ? 1 : -1)),
    ].map(menuItem => {
        const defaultLocale = process.env.NEXT_PUBLIC_DEFAULT_LOCALE || 'en';

        const targetUrl = menuItem.target.includes('http')
            ? menuItem.target
            : locale === defaultLocale
              ? menuItem.target
              : `/${locale}${menuItem.target}`;

        return (
            <MenuWidgetItem
                menuItem={menuItem}
                title={menuItem?.translations?.[locale]?.name || dictionary?.[menuItem.name] || menuItem.name || ''}
    ...
    return (
        <div
            ref={MenuWidgetRef}
            className={`menuWidget ${uniqueData?.burgerMenuOnDesktop ? '' : 'menuWidgetExpanded'} ${
        ...
        >
            ...
            <div className={'menuItemsWrapper custom-scroll'} style={{ zIndex: open ? 11 : 'initial' }}>
                <div className={'menuHeader'}>
                    ...
                    >
                        <FontAwesomeIcon
                            className={'menu-widget-close-button-logo'}
                            color={'var(--primary-text-color,#fff)'}
                            icon={faXmark}
                ...
                <div className="menuLinks">{renderMenuItems}</div>
            </div>
        </div>
    );
};
