          ...
          return (
            <div key={category?._id} className={'postsListByCategoriesGroup'}>
              <Link className={'categoryGroupTitleLink'} href={`/category/${category?._id}`}>
                ...
                {category?.postsCount > category.posts.length && <span>{dictionary?.['See All'] || 'See All'}</span>}
              </Link>
              ...
              <div className={'postsListByCategoriesWrapper custom-scroll'}>
                <ol className={'postsList'}>
                  {category.posts.map((post) => {
                    const postUrl = locale === defaultLocale ? `/post/promotion/${post._id}` : `/${locale}${`/post/promotion/${post._id}`}`;

                    return (
                      <li className={'postsListItem'} key={post._id}>
                        <Link href={post.redirectLink || postUrl} className={'postsListItemExternalLink'}>
