...
import { usernameValidatorRegisterForm, passwordValidatorRegisterForm, emailValidator } from '@repo/utils/dist/src';
...
const LoginRegisterPopupForms: FC<IProps> = ({ locale, dictionary }: IProps) => {
  ...
  useEffect(() => {
    setStateValidator({
      username: globalState.loginRegisterFormPopup === 'register' ? usernameValidatorRegisterForm(state?.username) : !!state?.username,
