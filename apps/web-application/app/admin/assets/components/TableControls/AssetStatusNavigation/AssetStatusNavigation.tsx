...
const AssetStatusNavigation: FC<{ statusesCount: any }> = ({ statusesCount }) => {
  ...
  const postsStatus =
    assetsType === 'posts' || assetsType === 'metas' ? ['all', ...postStatuses] : assetsType === 'users' ? ['all', ...userStatus] : [];
  ...
  const renderStatus = postsStatus.map((statusType: string) => {
    ...
    return (
      ...
      >
        ...
        <span className={'statusCount'}>{statusesCount?.[statusType] ? ` ${statusesCount[statusType]}` : ''}</span>
      </button>
    );
  });
