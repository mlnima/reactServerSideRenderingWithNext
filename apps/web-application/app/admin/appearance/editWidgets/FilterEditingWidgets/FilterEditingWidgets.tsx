...
const FilterEditingWidgets = () => {
  ...
  const availablePositions = useAppSelector(({ widgets }) => {
    return Object.keys(widgets?.adminPanelWidgets || []);
  });
  ...
  const renderPositionSelectors = availablePositions?.sort((a, b) => a.localeCompare(b))?.map(position => {

    return (
      <div className={`btn ${paramsPositions?.includes(position) ? 'btn-primary' : 'btn-dark'}`}
           key={position}
           title={position}
  ...
  return (
    <div id={'FilterEditingWidgets'} className={'position-selector'}>
      ...
      <button onClick={() => onSelectPositionHandler(availablePositions)}
              className={'btn btn-primary select-all'}>
        Show All
      </button>
      <button onClick={() => onSelectPositionHandler([])} className={'btn btn-primary unselect-all'}>Hide All</button>
