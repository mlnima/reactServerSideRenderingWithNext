...
const ItemPreview: FC<IProps> = ({ data, widgetData, activeEditingLanguage, setWidgetData, parentsOption }) => {
  ...
  const onEditHandler = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newData = [...widgetData.menuItems.slice(0, findItemIndex), itemData, ...widgetData.menuItems.slice(findItemIndex + 1)];
    setWidgetData({
      ...widgetData,
      menuItems: newData,
    });
  ...
  const onIndexChangeHandler = (action: boolean) => {
    const newDataToSet = {
      ...
      itemId: itemData.itemId || 100,
    };
    const findItemIndex = widgetData.menuItems.findIndex((i: IMenuItem) =>
      i.itemId ? i.itemId === newDataToSet.itemId : i.name === newDataToSet.name,
    );
    const newData = [...widgetData.menuItems.slice(0, findItemIndex), newDataToSet, ...widgetData.menuItems.slice(findItemIndex + 1)];
    setWidgetData({
      ...widgetData,
      menuItems: newData,
    });
  ...
  return (
    <div className="menuWidgetItemPreview" key={data?.itemId ? data.itemId.toString() + data.name : data.name}>
      <div className="menu-item-header">
        ...
        <button onClick={onOpenHandler}>{state.open ? 'close' : 'open'}</button>
      </div>
