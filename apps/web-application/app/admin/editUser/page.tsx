...
import { changePasswordAction, deleteUserAction, editUserDataAction, generateNewAPIKeyAction, getUserDataAction, updateUserDataAction } from "@storeDashboard/reducers/usersReducer";
...
const User = () => {
  ...
  const onPasswordResetHandler = () => {
    dispatch(changePasswordAction({ oldPass: resetPasswordData.oldPassword, newPass: resetPasswordData.newPassword1, newPass2: resetPasswordData.newPassword2 }));
  };
  ...
  return (
    <UserStyledDiv className='user-admin-edit-profile-page'>
      <div className='user-admin-edit-profile-page-section'>
        <p>Username :</p>
        <input className={'primaryInput'} name='username' value={userData?.username} onChange={e => onChangeHandler(e)} disabled={userData?.username === 'admin' || userData?.username === 'dashboard'} />
      </div>
      <div className='user-admin-edit-profile-page-section'>
        <p>Email :</p>
      ...
      <div className='user-admin-edit-profile-page-section'>
        <p>First Name :</p>
        <input className={'primaryInput'} name='firstName' value={userData?.firstName} onChange={e => onChangeHandler(e)} />
      </div>
      <div className='user-admin-edit-profile-page-section'>
        <p>Last Name :</p>
        <input className={'primaryInput'} name='lastName' value={userData?.lastName} onChange={e => onChangeHandler(e)} />
      </div>
      <div className='user-admin-edit-profile-page-section'>
        <p>Nick Name :</p>
        <input className={'primaryInput'} name='nickName' value={userData?.nickName} onChange={e => onChangeHandler(e)} />
      </div>
      <div className='user-admin-edit-profile-page-section'>
        <p>About :</p>
      ...
      <div className='user-admin-edit-profile-page-section'>
        <p>Role :</p>
        <select className={'primarySelect'} value={userData?.role} name='role' onChange={e => onChangeHandler(e)} disabled={userData?.keyMaster}>
      ...
      <div className='user-admin-edit-profile-page-section'>
        <p>Status :</p>
        <select className={'primarySelect'} value={userData?.status} name='status' onChange={e => onChangeHandler(e)} disabled={userData?.keyMaster}>
          {userStatus.map((status: string, index: number) => {
            return <option value={status} key={index}>{convertVariableNameToName(status)}</option>;
          })}
        </select>
      </div>
      <button className='btn btn-primary' onClick={() => dispatch(updateUserDataAction(userData))}>Save Changes</button>
      <div className='user-admin-edit-profile-page-section-reset-password'>
        <h2>Reset Password:</h2>
        <p>Old Password:</p>
        <input className={'primaryInput'} name='oldPassword' value={resetPasswordData.oldPassword} onChange={e => onPasswordDataChangeHandler(e)} />
        <p>New Password:</p>
        <input className={'primaryInput'} name='newPassword1' value={resetPasswordData.newPassword1} onChange={e => onPasswordDataChangeHandler(e)} />
        <p>Repeat New Password:</p>
        <input className={'primaryInput'} name='newPassword2' value={resetPasswordData.newPassword2} onChange={e => onPasswordDataChangeHandler(e)} />
