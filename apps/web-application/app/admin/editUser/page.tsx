    ...
    <div className="EditUserPage">
      ...
      <div className="user-admin-edit-profile-page-section">
        ...
        >
          {userRoles.map((userRole: string, index: number) => {
            return (
              <option value={userRole} key={index}>
                {convertVariableNameToName(userRole)}
              </option>
            );
          })}
        </select>
          ...
          {userStatus.map((status: string, index: number) => {
            return (
              <option value={status} key={index}>
                {convertVariableNameToName(status)}
              </option>
            );
          })}
        </select>
      </div>
      <button className="btn btn-primary" onClick={onUpdateUserHandler}>
        Save Changes
      </button>
      <div className="user-admin-edit-profile-page-section-reset-password">
        ...
        <input
          className={'primaryInput'}
          name="newPassword1"
          value={resetPasswordData.newPassword1}
          onChange={(e) => onPasswordDataChangeHandler(e)}
        />
        ...
        <input
          className={'primaryInput'}
          name="newPassword2"
          value={resetPasswordData.newPassword2}
          onChange={(e) => onPasswordDataChangeHandler(e)}
        />
        <button className="saveBtn btn btn-primary" onClick={onPasswordResetHandler}>
          Reset The Password
        </button>
      </div>
      <div className="user-admin-edit-profile-page-section-API">
        ...
        <button className="btn btn-primary" onClick={onNewAPIKeyRequest}>
          Generate API Key
        </button>
      </div>
      {!userData?.keyMaster ? (
        <div className="user-admin-edit-profile-page-delete-user">
          <h2>Delete User</h2>
          <button className="delete-user-button" onClick={onDeleteUserHandler}>
            Delete User
          </button>
        </div>
      ) : null}
