...
import { postSchema, metaSchema, widgetSchema } from '@repo/db';

const updatePostsListEntireByCategoriesWidget = async (widgetData: any, widgetId: string, res: Response) => {
  try {
    const categories = await metaSchema
      .find({
        $and: [{ type: 'categories' }, { status: 'published' }],
      })
      .sort({ count: -1 })
    ...
    const categoriesCount = await metaSchema
      .countDocuments({
        $and: [{ type: 'categories' }, { status: 'published' }],
      })
      .exec();
    ...
    const fetchPostsForCategory = async (category: any) => {
      const postsOfCurrentCategory = await postSchema
        .find({
      ...
      const postsCount = await postSchema
        .countDocuments({
          categories: category._id,
        })
        .exec();
