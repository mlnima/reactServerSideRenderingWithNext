...
const searchAndScrapLink = async ({ keyword, page }: ISearchAndScrapLink) => {
    try {
        ...
        return await got(targetUrl)
            .then(async searchPageData => {
                const searchPageDom = new JSDOM(searchPageData.body.toString())
                    .window.document;
                const allVideoCards =
                    searchPageDom.querySelectorAll('.video-thumb');

                for await (const card of allVideoCards) {
                    const UHDQualityElement = card.querySelector(
                        '.thumb-image-container__icon--uhd',
                    );
                    const HDQualityElement = card.querySelector(
                        '.thumb-image-container__icon--hd',
                    );
                    ...
                    const cardData = {
                        ...
                        mainThumbnail: card
                            ...
                            ?.querySelector('img')
                            ?.getAttribute('src'),
                        videoTrailerUrl: card
                            .querySelector('a')
                            ?.getAttribute('data-previewvideo'),
                        views: card.querySelector('.video-thumb-views')
                            ?.textContent,
                        quality: !!UHDQualityElement
                            ? '4K'
                            : !!HDQualityElement
                              ? 'HD'
                              : 'SD',
                        ...
                        title: card
                            .querySelector('.video-thumb-info')
                            ?.querySelector('.video-thumb-info__name')
                            ?.textContent,
                    };

                    if (
                        // cardData?.duration &&
                        cardData?.videoTrailerUrl &&
