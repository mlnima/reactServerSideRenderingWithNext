...
const authWithUserDataMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const user = await userSchema.findById(decodedToken._id).select('username role keyMaster').lean().exec();
