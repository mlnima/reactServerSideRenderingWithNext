...
const updateMetasWidget = async (widgetData: any,widgetId:string, res: Response) => {
    ...
    try {
        ...
        const limit = widgetData?.count || widgetData?.uniqueData?.count || global?.initialSettings?.postCardsSettings?.numberOfCardsPerPage || 20;
        const sortQuery = widgetData?.sort || widgetData?.uniqueData?.sort ? { 'updatedAt': -1 } : { 'rank': 1, 'count': -1 };
