            ...
            for await (const metaType of metaTypes) {
                ...
                for await (const currentPage of amountOfPages) {
                    ...
                    fs.writeFileSync(
                        ...
                        SitemapController.urlSetXmlTemplate(SitemapController.metaXmlTemplateGenerator(metas , '1', 'hourly', convertMetasTypeToSingular(metaType))),
                        {
                            encoding: "utf8",
                            flag: "w",
                        })
    ...
    static async postSitemapLinkForTheRoot(){
        try {
            ...
            const rangeOfTheMonths = firstPostDate === lastPostDate ?
                [firstPostDate] :
                SitemapController.getRangeOfTheDates(firstPostDate, lastPostDate)
                ...
                if (postCountInThisMonth){
                    ...
                    for await (const currentPage of amountOfPages) {
                        try {
                            ...
                            const lastUpdate = new Date(SitemapController.mongoDocumentDateCorrector(lastDocumentUpdatedDate.createdAt, lastDocumentUpdatedDate._id));
    ...
    static async pagesSitemapGenerator(){
        try {
            ...
            fs.writeFileSync(
                ...
                SitemapController.pagesTemplateGenerator(pages),
                {
                    encoding: "utf8",
                    flag: "w",
                })
