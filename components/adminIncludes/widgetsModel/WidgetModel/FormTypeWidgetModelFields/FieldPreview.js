...
const FieldPreview = props => {
        ...
        const onDeleteHandler = name => {
            const newData = props.widgetData?.uniqueData?.formFields?.filter(i => i.fieldName !== name)
            props.setWidgetData({
                ...props.widgetData,
                uniqueData: {
                    ...(props.widgetData?.uniqueData || {}),
                    formFields: newData
                }
            })
        }
        const onEditHandler = e => {
            e.preventDefault()
            const findIndexOfTheField = props.widgetData?.uniqueData?.formFields?.findIndex(f => f.fieldName === props.field.fieldName)
            const updatedFields = [
                ...props.widgetData?.uniqueData?.formFields?.slice(0, findIndexOfTheField),
                fieldData,
                ...props.widgetData?.uniqueData?.formFields?.slice(findIndexOfTheField + 1),
            ];
            props.setWidgetData({
                ...props.widgetData,
                uniqueData: {
        ...
        const fieldIndexPlus = value => {
            ...
            const findIndexOfTheField = props.widgetData.formFields.findIndex(f => f.filedId === props.field.filedId)
            const updatedFields = [
                ...props.widgetData.formFields.slice(0, findIndexOfTheField),
                updatedFieldData,
                ...props.widgetData.formFields.slice(findIndexOfTheField + 1),
            ];
            props.setWidgetData({
                    ...props.widgetData,
                    uniqueData: {
            ...
            <StyledDiv className='form-item-view' key={props.field.filedId}>
                <div className='field-index-control'>
                    <button
                        ...
                        <FontAwesomeIcon
                            style={{transform: state.open ? 'rotate(90deg)' : 'rotate(0deg)', transition: '.5s all'}}
                            icon={faBars}/>
                    </button>
                    <p>{'ID : ' + props.field.filedId}</p>
                    <p>{props.field.fieldType + ' : ' + props.field.fieldName}</p>
                    <p>index:{props.field.fieldIndex}</p>
                    <button onClick={() => fieldIndexPlus(-1)} className={'btn btn-secondary'}><FontAwesomeIcon
                        icon={faArrowUp}/></button>
                    <button onClick={() => fieldIndexPlus(+1)} className={'btn btn-secondary'}><FontAwesomeIcon
                        icon={faArrowDown}/></button>
                    <button onClick={() => onDeleteHandler(props.field.fieldName)} className={'btn btn-secondary'}>
