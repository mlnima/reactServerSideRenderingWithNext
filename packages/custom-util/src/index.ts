export {default as googleAnalyticsEvent} from './analytics-utils/google-analytics/googleAnalyticsEvent';
export {default as googleAnalyticsPageView} from './analytics-utils/google-analytics/googleAnalyticsPageView';
export {default as headerSizeCalculator} from './vanilla-ui-utils/headerSizeCalculator';
export {default as inputValueSimplifier} from './inputsUtils/inputValueSimplifier';
export {default as shortNumber} from './math-utils/shortNumber';
export {default as arraySortRandom} from './array-utils/arraySortRandom';
export {default as groupingArrayOfObjectByKey} from './array-utils/groupingArrayOfObjectByKey';
export {default as capitalizeFirstLetter} from './string-util/capitalizeFirstLetter';
export {default as base64toBlobURL} from './string-util/base64toBlobURL';
export {default as capitalizeFirstLetters} from './string-util/capitalizeFirstLetters';
export {default as convertVariableNameToName} from './string-util/convertVariableNameToName';
export {default as fileTypeDetector} from './string-util/fileTypeDetector';

export {default as isAbsolutePath} from './url-util/isAbsolutePath';
export {default as languageDetectorFromURL} from './url-util/languageDetectorFromURL';
export {default as doesUrlHasQueries} from './url-util/doesUrlHasQueries';
export {default as postCanonicalUrlGenerator} from './url-util/postCanonicalUrlGenerator';
export {default as postsCanonicalUrlGenerator} from './url-util/postsCanonicalUrlGenerator';
export {default as metasCanonicalUrlGenerator} from './url-util/metasCanonicalUrlGenerator';
export {default as customPageCanonicalUrlGenerator} from './url-util/customPageCanonicalUrlGenerator';
export {default as isEmptyObject} from './object-util/isEmptyObject';
export {default as isImageAllowedForNextImage} from './url-util/isImageAllowedForNextImage';
export {default as isInternalUrl} from './url-util/isInternalUrl';
export {default as isJsonString} from './string-util/isJsonString';
export {default as isNumericString} from './string-util/isNumericString';
export {default as queryUniquer} from './url-util/queryUniquer';

export {default as mongoIdValidator} from './string-util/mongoIdValidatorClient';
export {default as randomNumberGenerator} from './math-utils/randomNumberGenerator';
export {default as rangeNumGenerator} from './math-utils/rangeNumGenerator';
export {default as ratingCalculator} from './math-utils/ratingCalculator';
export {default as uniqArrayBy} from './array-utils/uniqArrayBy';
export {default as reduceArrayOfDataToIds} from './array-utils/reduceArrayOfDataToIds';
export {default as reduceWidgetsToGroups} from './array-utils/reduceWidgetsToGroups';
export {default as textContentReplacer} from './string-util/textContentReplacer';
export {default as getTextDataWithTranslation} from './object-util/getTextDataWithTranslation';
export {default as removeEmptyProperties} from './object-util/removeEmptyProperties ';
export {default as convertMetasTypeToPlural} from './string-util/convertMetasTypeToPlural';
export {default as convertMetasTypeToSingular} from './string-util/convertMetasTypeToSingular';
export {default as sortArrayByPropertyOfObject} from './array-utils/sortArrayByPropertyOfObject';
export {default as convertSecondsToTimeString} from './date-utils/convertSecondsToTimeString';
export {default as convertDateToIsoString} from './date-utils/convertDateToIsoString';
export {default as convertDurationStringToIso8601} from './date-utils/convertDurationStringToIso8601';
export {default as touchDeviceDetector} from './device-util/touchDeviceDetector';
export {default as usernameValidatorRegisterForm} from './validators/usernameValidatorRegisterForm';
export {default as passwordValidatorRegisterForm} from './validators/passwordValidatorRegisterForm';
export {default as removeFileExtension} from './file-utils/removeFileExtension';
export {default as blobToBase64} from './file-utils/blobToBase64';
export {default as imageCanvasCompressor} from './file-utils/imageCanvasCompressor';
export {default as ImageCompressorByBrowser} from './file-utils/ImageCompressorByBrowser';
export {default as emailValidator} from './validators/emailValidator';

export {default as renderByLanguageCondition} from './widgets-util/renderByLanguageCondition';
export {default as renderByDayCondition} from './widgets-util/renderByDayCondition';
export {default as renderByDevice} from './widgets-util/renderByDevice';
export {default as isEditMode} from './widgets-util/isEditMode';

//translation
export {default as nextTranslateWithCallback} from './translation-utils/nextTranslateWithCallback'
//jsx
export {default as LanguagesOptions} from './jsx-utils/LanguagesOptions'
