...
export const clientAPIRequestUploadPostImages = async (formData: FormData) => {
    return await FileServerAxiosInstance.post(`/files/v1/upload/uploadPostImages`, formData, {
        headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${localStorage.wt}`,
        },
...
export const clientAPIRequestUploadProfileImage = async (formData: FormData) => {
    return await FileServerAxiosInstance.post(`/files/v1/upload/uploadProfileImage`, formData, {
        headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${localStorage.wt}`,
        },
...
export const clientAPIRequestDeletePostImage = async ({postId, imageId, thumbnailToReplace}: IDeletePostImage) => {
    ...
    return await FileServerAxiosInstance.delete(`/files/v1/upload/deletePostImage?${queries}`);
}
...
export const clientAPIRequestDeletePostImages = async ({postId}: IDeletePostImages) => {
    return await FileServerAxiosInstance.delete(`/files/v1/upload/deletePostImages?postId=${postId}`);
}
